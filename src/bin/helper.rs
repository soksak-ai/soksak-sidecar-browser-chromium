...
// 패턴). framework 로드 + execute_process. 브라우저(메인) 프로세스는 앱 본체(dylib 의 initialize)가
// 담당하며 이 바이너리를 직접 실행하지 않는다.
//
...
//
// 배치: dist/soksak-sidecar-browser-chromium Helper.app/Contents/MacOS/<이 바이너리>. LibraryLoader(helper=true)
// 가 실행파일 기준 ../../../../Chromium Embedded Framework.framework 를 해소한다.
...
fn main() {
    eprintln!("soksak-sidecar-browser-chromium-helper: macOS 전용");
    std::process::exit(1);
}
