// offscreen 호스팅 모드의 프레젠터 — 엔진(Chromium)이 offscreen 렌더한 공유 텍스처(IOSurface)를
// 모듈 소유 layer-hosting NSView 의 CALayer 로 present 한다. 픽셀은 이 파일 밖으로 나가지 않는다
// (SIDECARS.md §8 offscreen 규칙: 프레임 데이터의 호스트 vtable/IPC/JS 통과 금지).
//
...
pub(crate) fn present(id: u32, cef_surface: *mut c_void, coded_w: i32, coded_h: i32) {
    present_target(id, cef_surface, coded_w, coded_h, false);
}

// PET_POPUP 프레임 — 팝업 서브레이어로 합성(스펙 §8 M4).
pub(crate) fn present_popup(id: u32, cef_surface: *mut c_void, coded_w: i32, coded_h: i32) {
    present_target(id, cef_surface, coded_w, coded_h, true);
}
