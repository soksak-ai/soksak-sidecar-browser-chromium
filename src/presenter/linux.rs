// offscreen 프레젠터 — Linux 구현. CEF 가 offscreen 렌더한 공유 텍스처(DMA-BUF plane fd)를 EGLImage
// (EGL_EXT_image_dma_buf_import)로 가져와 GL 텍스처로 바인딩, 모듈 소유 child X11 창의 GL 표면에 합성한다.
// 구조는 offscreen.rs(macOS reference)를 미러한다: 서피스 레지스트리 · 3버퍼 풀 · 팝업 서브레이어 ·
// present 스왑. GPU/윈도우 API 만 EGL/DMA-BUF·X11 로 교체하고 상태 계약(논리크기·scale·hidden·popup)은 동일.
//
// 진행 단계: 레지스트리·상태 부기는 완결(is_offscreen/logical_size/scale_of/set_bounds/set_hidden/
// destroy/popup_*). 네이티브 child 창 생성·DMA-BUF EGL import·present 는 Phase C/D(CI 검증, xvfb). 미구현
// 경로는 조용히 성공하지 않고 log_once 로 표식한다(SIDECARS.md P 규칙: 조용한 강등 금지).
...
pub(crate) fn create_surface(id: u32, parent: usize, _x: i32, _y: i32, w: i32, h: i32, scale: f32) {
    ...
    log_once(id, "linux offscreen child 표면 생성 미구현 (Phase C/D: X11 child + EGL)");
}
...
pub(crate) fn present(id: u32, info: &cef::AcceleratedPaintInfo) {
    let _ = info; // Phase C/D: info.planes/plane_count (DMA-BUF fd) → EGLImage → GL 합성
    log_once(id, "linux present 미구현 (Phase C/D: DMA-BUF plane fd → EGLImage → GL 합성)");
}

pub(crate) fn present_popup(id: u32, info: &cef::AcceleratedPaintInfo) {
    let _ = info;
    log_once(id, "linux 팝업 present 미구현 (Phase C/D)");
}
