// offscreen 프레젠터 — Linux 구현. CEF 공유 텍스처(DMA-BUF)를 cef 크레이트의 osr_texture_import
// (accelerated_osr 피처, wgpu 29)로 wgpu::Texture 로 가져와, 모듈 소유 child X11 창의 wgpu::Surface 에
// 렌더한다. 세 네이티브 GPU 스택을 손으로 굴리지 않고 크레이트의 통합 임포터를 쓴다(정석). windows 와
// present 메커니즘(wgpu) 공유 — macOS(raw Metal, offscreen.rs) 만 별개. 상태 계약(레지스트리·논리크기·
// scale·hidden·popup)은 offscreen.rs 를 미러한다.
//
...
pub(crate) fn create_surface(id: u32, parent: usize, _x: i32, _y: i32, w: i32, h: i32, scale: f32) {
    ...
    log_once(id, "linux offscreen child 표면 생성 미구현 (Phase C/D: X11 child + wgpu::Surface)");
}
...
pub(crate) fn present(id: u32, info: &cef::AcceleratedPaintInfo) {
    ...
    log_once(id, "linux present 미구현 (Phase C/D: osr_texture_import → wgpu::Texture → wgpu::Surface)");
}
